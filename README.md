...
- **💬 라이브 채팅 모드** — 켜면 `!참여`한 시청자의 채팅 문구가 그대로 제시어가 됩니다. **홈 참여자 명단 옆 버튼**으로 방송 세팅 중에 켜고, 상단 컨트롤바 버튼으로 **게임 중에도 즉시 ON/OFF**(두 버튼 실시간 동기화). 이모티콘·특수문자·비속어를 걸러 안전하게 사용하며, 제시어 길이는 6자로 고정됩니다.
