...
- **토스 후원 모달**: 상단 `[☕ 후원]` 버튼을 눌러 **후원 금액을 선택하거나 직접 입력** 후 `[💳 토스(Toss)로 후원하기]` 버튼을 클릭하면 토스 결제 페이지로 바로 이동합니다. 계좌번호 등 개인정보는 일절 노출·저장되지 않습니다.

### 16. 📱 반응형 UI & 멀티 디스플레이 최적화 (Responsive UI)
...
- [`index.html`](./index.html) : 메인 레이아웃, N인 선택 UI, 난이도 선택, 광고 배너 슬롯, 토스 후원 모달, 등급 뱃지 및 결과 랭킹
- [`style.css`](./style.css) : 사이버펑크 네온 글로우 CSS, N인 그리드, 반응형 미디어 쿼리, 광고 슬롯 및 후원 모달 스타일
- [`js/config.js`](./js/config.js) : 민감 정보(토스 ID, 광고 ID) 전용 설정 파일 (공개 금지 권장)
- [`js/game.js`](./js/game.js) : Canvas 2D 파티클, Stage/웨이브 엔진, 등급 산정, localStorage 명예의 전당, 토스 후원 링크 로직
