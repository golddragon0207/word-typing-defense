...
- **참여자 목록 & 재모집**: 홈 화면 채팅 연동 패널에서 `!참여`한 시청자를 실시간 목록으로 확인. **🗑️ 참여자 초기화** 버튼으로 언제든 비울 수 있습니다. **`🔄 다시 도전하기`를 누르면 메인 화면으로 돌아가며 홈의 채팅 연동 패널이 자동으로 펼쳐져 시청자를 다시 모집**합니다(방송 URL·연결은 그대로 유지). 판이 끝나 메인으로 돌아갈 때 명단은 자동으로 초기화됩니다.
...
- 스테이지가 오를수록 스폰 주기가 짧아지고 낙하 속도가 점점 빨라집니다.
