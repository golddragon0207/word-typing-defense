...
### 15. 💰 웹 게임 수익화 & ☕ 계좌 후원 연동
- **카카오 애드핏 (Kakao AdFit)**: 실시간 광고 스크립트(`DAN-DtBvl47G2a1Jbre4`)가 메인 화면, 결과 창, 명예의 전당 하단 배너 슬롯에 탑재되어 실시간 광고 수익 창출이 가능합니다.
- **구글 애드센스 (Google AdSense)**: 구글 애드센스 퍼블리셔 ID(`ca-pub-1454462720476509`) 사이트 소유권 연동 코드가 포함되어 있습니다.
- **카카오뱅크 후원 카카오 모달**: 상단 `[☕ 후원]` 버튼을 눌러 **카카오뱅크 `3333-28-2684443` (예금주: 김영석)** 계좌를 확인하고 `[📋 계좌번호 복사]` 버튼으로 1초 만에 복사할 수 있습니다.
...
- [`index.html`](./index.html) : 메인 레이아웃, N인 선택 UI, 난이도 선택, 광고 배너 슬롯, 카카오뱅크 후원 모달, 등급 뱃지 및 결과 랭킹
- [`style.css`](./style.css) : 사이버펑크 네온 글로우 CSS, N인 그리드, 반응형 미디어 쿼리, 광고 슬롯 및 후원 모달 스타일
- [`js/game.js`](./js/game.js) : Canvas 2D 파티클, Stage/웨이브 엔진, 등급 산정, localStorage 명예의 전당, 계좌 복사 토스트
