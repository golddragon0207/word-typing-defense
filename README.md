...
- **다중 HP 보스**: 보스는 여러 번 격파해야 처치됩니다. HP는 스테이지가 오를수록 완만히 증가(`2 + floor(stage/15)`, 상한 5 → Stage 5·10=2방, 15~=3방, 30~=4방, 45~=5방). **격파할 때마다 새 보스 제시어로 교체**되고 붉은 피격 플래시가 터지며, 머리 위 **HP 하트(pip)** 로 남은 횟수를 시청자도 한눈에 봅니다. 보스 피격은 점수·콤보·타수에 반영되지만 **처치 수는 완전히 잡았을 때 1회만** 집계됩니다. 낙하 속도는 스테이지에 따라 빨라지고, 뚫렸을 때 관통 피해도 완만히 커집니다(`3 + floor(stage/20)`배).
