...
### 8. 🖥️ 1024×768 고정 무대 + 비율 스케일 & OBS 크로마키
- **내부 논리 해상도를 1024×768로 고정**하고, 무대 전체를 창 크기에 맞춰 **비율을 유지한 채 확대/축소(scale-to-fit)**합니다. 게임 좌표·방어선·몬스터 위치가 창 크기와 무관하게 **절대 리플로우되지 않아**, 창을 키웠다 줄이거나 다른 탭을 다녀와도 게임이 깨지지 않습니다.
...
- [`style.css`](./style.css) : 사이버펑크 네온 CSS, **1024×768 고정 무대 + 비율 스케일(scale-to-fit)** 레이아웃, 반응형 상단바(아이콘 접힘 + 호버 툴팁), 모달/토스트/등급뱃지 스타일, OBS 투명 오버레이
