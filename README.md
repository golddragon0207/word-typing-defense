...
- **상단 뱃지**: 실제 참여 시청자 닉네임(`🟢치지직`, `🔵SOOP`, `🔴유튜브` 접두사) 또는 `[BOT]` 표식.
...
- 기본은 안전한 단어팩에서 제시어를 뽑지만, 상단 **`💬 라이브 채팅 모드`** 버튼을 켜면 **`!참여`한 시청자의 이후 채팅 문구**가 그대로 타이핑 제시어가 됩니다.
- 게임 중에도 즉시 ON/OFF. 이모티콘·특수문자 제거, 비속어 필터, 최대 글자수 컷(모달에서 설정) 후 안전하게 사용합니다.
...
- 자율 커스텀 단어장 등록(단어당 최대 16자, 최대 300개, 비속어 자동 필터).

### 10. 💰 수익화 광고 & ☕ 후원
- 카카오 애드핏 `728×90` 배너 6개 슬롯(메인/결과/각 모달). 모달 오픈 시 동적 리프레시.
- 상단 `☕ 후원` → 네이버(크티) QR 후원 모달(계좌 등 개인정보 미노출).
...
- [`js/wordPacks.js`](./js/wordPacks.js) : 단어팩(프리셋/커스텀/보스), 시청자 대기열, `!참여` 처리·봇 보충, 라이브 채팅 정제, 한글 자모 획수 유틸
