...
- **♾️ 무한 스테이지 & 보스전** — 패배할 때까지 무한 진행, 5스테이지마다 **차지(기 모으기) 보스** 등장. 게이지가 차기 전에 정타로 격파하는 방식이라 클리어는 보장되되 느릴수록 데미지를 더 맞습니다. 보스 공격에 기지가 피격당하면 화면 가장자리에 **붉은 위험 비네트 + 피해량** 연출로 즉시 알려줍니다.
