...
- **📖 홈 화면 게임 설명서** — 홈 상단에 접이식 설명서 패널이 있어, 처음 온 사람도 "어떤 게임인지 / 스트리머는 어떻게 시작하는지(연동→닉네임→시작 3단계) / 시청자는 `!참여`로 어떻게 들어오는지"를 한눈에 봅니다. 기본은 접혀 있어 화면을 가리지 않고, 좁은 화면에선 역할 카드가 세로로 재배치됩니다.
