...
*  **봇 자동 보충(`topUpBotsToTarget`)**: 실참여자(명단)+봇을 합쳐 목표 인원(`TARGET_MIN_POPULATION`, 기본 20)까지만 유지. 봇은 `목표 − 실참여자 수`만큼만 채워지므로 실참여자가 많을수록 봇이 자동으로 줄고, 실참여자가 목표 이상이면 봇은 0.
