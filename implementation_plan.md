...
*  IME 조합 완료 감지 후, **타수(`getKeystrokeCount`)** 누적으로 CPM/WPM 산출 → HUD 실시간 표시. **한컴 타자연습 "자소 단위"(2024.4~) 방식과 일치** — 자소 하나당 1타(쌍자음 ㄲ·ㅒㅖ은 Shift로 쳐도 1타), 겹모음(ㅘ 등)·겹받침(ㄳ 등)만 2타. 획수 기반 `getHangulStrokeCount`는 표시 타수가 실제보다 ≈2배 부풀려져 혼동을 주므로 표시용에서 분리했다. **점수 계산은 여전히 획수 기반**(`getHangulStrokeCount`)이라 난이도 비례 점수는 그대로 유지 — 타수 표시(자소)와 점수(획수)는 별개 단위.
