...
*  **토글 위치**: 상단 컨트롤바 `💬 라이브 채팅 모드` 버튼으로 **게임 중에도 즉시 ON/OFF**(OBS·사운드 토글과 동일한 라이브 컨트롤 성격). **클릭 후 버튼 포커스 해제**(`_blurQuickControl`, 라이브/OBS/사운드 공용) — 마우스로 켠 뒤 제시어 입력하려 친 Enter가 버튼을 재클릭해 모드가 뒤집히던 문제를 막고, 게임 중이면 입력창으로 포커스를 되돌린다.
...
*  **기본 제시어 풀(`wordPacks.words`) 다양화**: 반복 체감을 줄이기 위해 방송/게임/개발/음식·일상/밈/자연 등 여러 주제를 섞어 구성(약 95개, 짧은 2~3자부터 중간 길이까지). 정적 사이트라 별도 JSON/`fetch` 없이 이 배열만 늘리면 즉시 반영된다.
...
*  **완만한 난이도 곡선**: 스테이지 간 난이도 점프를 최소화하고 **후반에는 요구 타수가 평탄해지도록**(일반 스테이지가 항상 직전~다음 보스보다 낮게 유지 → 보스가 각 스테이지보다 "조금 더" 어렵게) 낙하·스폰·처치수를 튜닝(실제 단어팩 획수 분포 + 현실적 플레이 모델로 시뮬레이션 검증).
   ...
   *  **검증(목표 타수 = 실제 키 입력 기준, normal)**: 일반 S1~50 · S2~55 · S3~65 · S4~80 → S9경 ~130에서 평탄. (보스 요구 타수는 §보스 항목 참조 — 항상 직전 스테이지보다 +10~35.)
...
*  **HUD 상태바는 무대 상단에 부착**: `#game-hud`(STAGE/SCORE/타수/COMBO/체력/FEVER)를 `top:0` 전체 폭 띠로 두고 하단 네온 경계선으로 상단바와 이어지게 표시(z-index 10으로 캔버스 위에 겹쳐 그려짐). 몬스터는 이 상태바(스테이지창, 0~71px) '안'(`y:40`)에서 생성돼 상태바 뒤에 잠깐 가려진 채 시작해 아래로 스르륵 내려온다(스테이지창부터 떨어지는 연출 + 낙하 구간 확보). 출전 대기열 패널은 `top:80`으로 내려 HUD와의 세로 겹침을 피하고, **몬스터 스폰 x좌표도 패널 우측끝(≈183px)을 침범하지 않도록 좌측을 확보**한다(`spawnMonster`가 닉네임/제시어 길이로 박스 폭을 추정해 중심 x의 최소값을 `190 + 박스폭/2`로 잡음) — 좌상단 대기열 패널에 몬스터가 가려지지 않는다.

### 10. ⚡ 방어 속도(타수 지표) & 콤보 & 피버
*  IME 조합 완료 감지 후, **타수(`getKeystrokeCount`)** 누적을 **총 경과시간(대기 시간 포함)으로 나눠** CPM/WPM 산출 → HUD **"방어 속도"**로 실시간 표시.
   *  **자소 단위 계산(한컴 타자연습 2024.4~ 방식과 일치)**: 자소 하나당 1타(쌍자음 ㄲ·ㅒㅖ은 Shift로 쳐도 1타), 겹모음(ㅘ 등)·겹받침(ㄳ 등)만 2타. 획수 기반 `getHangulStrokeCount`는 표시값이 실제보다 ≈2배 부풀려져 혼동을 주므로 타수 표시용에서 분리.
   *  **명칭이 "타수"가 아니라 "방어 속도"인 이유**: 분모가 순수 타이핑 시간이 아니라 **몬스터 대기·카운트다운 등 노는 시간까지 포함한 판 전체 시간**이라, 한컴 시험처럼 쉬지 않고 친 순수 타자 속도보다 낮게 나온다. "이 판에서 낸 평균 방어 속도" 지표이지 순수 타수 기록이 아님을 이름으로 명확히 함(HUD·결과 화면·명예의 전당 모두 동일 명칭).
   *  **점수 계산은 여전히 획수 기반**(`getHangulStrokeCount`)이라 난이도 비례 점수는 그대로 유지 — 방어 속도(자소)와 점수(획수)는 별개 단위.
...
*  **⏸ 일시정지**(`GameEngine.togglePause`): 게임 진행 중 **ESC 키** 또는 **마우스(HUD 우측 `#btn-pause` / 오버레이 `#btn-pause-resume`)** 로 정지/재개(모달 열림 시 무시). 정지 중 몬스터 이동·스폰·입력을 멈추고 오버레이(`#pause-overlay`) 표시. **정지 동안 흐른 시간을 `startTime`에 더해** 경과시간(=WPM 분모)에서 제외 → 화장실 등으로 잠깐 비워도 타수가 왜곡되지 않음.
    *  **정지 중 스폰 완전 차단**: 주기 스폰(`_spawnTick`)뿐 아니라 **시작 그레이스·스테이지업 지연·보스 소환 지연 타이머(`setTimeout`)가 정지 중에 발화**해도 몬스터가 튀어나오지 않도록 `spawnMonster`/`spawnBoss` 진입부에서 `isPaused`를 직접 가드한다(보스는 `bossSpawnedForStage`로 이중 소환도 방지). 이 타이머들은 정지와 무관하게 발화하므로 함수 레벨 가드가 없으면 정지 상태에서 몬스터/보스가 스폰되던 버그가 있었다.
    *  **재개 시 첫 등장 복구**(`MonsterManager.resumeSpawns`): 정지 중 스킵된 '스테이지 첫 몬스터/보스'를 재개 시 이어서 소환한다(일반: 스폰 시작됐는데 화면이 비면 1마리 / 보스: 소환 타이머가 이미 발화했는데 보스가 없으면 소환) → 재개 후 빈 화면 방지.
