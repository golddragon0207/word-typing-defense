...
> **v3.5 변경 요약**: **차지(기 모으기) 보스전으로 전면 개편** — 낙하 다중 HP 보스(v3.4)가 긴 보스 제시어(평균 43타)와 겹쳐 물리적으로 클리어 불가였던 문제(시뮬레이션 검증)를 해결. 보스는 낙하하지 않고 고정 위치에서 차지 게이지를 채우며, 게이지가 차기 전 정타로 격파하면 게이지 절반 밀어내기+새 제시어 교체, 필요 격파 수 달성 시 처치. 게이지 만땅 시 보스 공격(기지 정액 피해)+게이지 리셋(진행도 유지=항상 클리어 보장, 소프트락 없음). 스테이지 비례 스케일: 필요격파 `2+floor(st/20)`(≤5), 차지시간 `필요격파×6초`, 공격력 `10+floor(st/15)×3`, 후반 긴 제시어 우선. 렌더러에 차지 게이지 바+남은 격파 pip+공격 플래시. 시뮬레이션상 전 스테이지 클리어율 100%·보스 단독 사망 0.
