...
*  **⏸ 스페이스바 일시정지**(`GameEngine.togglePause`): 게임 진행 중 스페이스바로 정지/재개(모달 열림 시 무시). 정지 중 몬스터 이동·스폰(`_spawnTick` 가드)·입력을 멈추고 오버레이(`#pause-overlay`) 표시. **정지 동안 흐른 시간을 `startTime`에 더해** 경과시간(=WPM 분모)에서 제외 → 화장실 등으로 잠깐 비워도 타수가 왜곡되지 않음. 단어에 공백이 없어 입력창의 스페이스를 가로채도 타이핑에 지장 없음.
