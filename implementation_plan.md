...
*  IME 조합 완료 감지 후, **실제 키 입력 타수(2벌식, `getKeystrokeCount`)** 누적으로 CPM/WPM 산출 → HUD 실시간 표시. 한컴 타자연습식 타수와 같은 단위라 사용자에게 직관적(획수 기반 `getHangulStrokeCount`는 표시 타수가 실제보다 ≈2배 부풀려져 혼동을 주므로 표시용에서 분리). **점수 계산은 여전히 획수 기반**(`getHangulStrokeCount`)이라 난이도 비례 점수는 그대로 유지 — 타수 표시(키 입력)와 점수(획수)는 별개 단위.
