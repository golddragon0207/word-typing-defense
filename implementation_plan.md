...
* **세부 설정**: 단어/닉네임 팩 모달에서 최대 글자수(4/6/8, 기본 6 — 몬스터 4자 밸런스에 맞춰 짧게)·특수문자 제거 여부 설정. 라이브 채팅 설정 박스는 **모드 ON일 때만 보라색 테두리+글로우로 활성 표시**(`.live-active` 클래스로 양쪽 토글 동기화). 상태 전환 토스트는 🟢 ON / 🔴 OFF 표기.
