# 🎮 word-typing-defense (스트리머 워드 디펜스) 구현 계획서 (v3.4)
...
> **v3.4 변경 요약**: **다중 HP 보스전 도입** — 보스가 여러 번 격파해야 죽도록 변경(`hp = 2 + floor(stage/15)`, 상한 5). 격파 시마다 새 보스 제시어로 교체 + 붉은 피격 플래시, 머리 위 HP 하트(pip) 표시. 보스 피격은 점수·콤보·타수에 반영하되 처치 수는 완전 처치 시 1회만 집계(`StateManager.registerHit(word, score, countKill)` 파라미터 추가, `MonsterManager.checkHit`에서 보스 HP>1이면 제거 대신 감소·리롤). 보스 관통 피해도 스테이지 비례 완만 증가(`3 + floor(stage/20)`배).
>
...
*  **다중 HP 보스**(`spawnBoss`): `hp = 2 + floor(stage/15)`(상한 5). `checkHit`에서 보스 HP가 남아 있으면 배열에서 제거하지 않고 `hp -= 1` + 직전과 다른 새 제시어로 리롤 후 `{ isKilled:false, bossDamaged:true, remainingHp }` 반환. HP가 1일 때의 타격에서만 실제 제거·`isKilled:true`. `game.js`는 `bossDamaged`면 `registerHit(word, score, false)`로 **점수·콤보·타수만 반영(처치 수 미증가)** + 피격 사운드 + `_flashUntil` 설정, 완전 처치 시에만 폭발·`advanceStage`. 렌더러는 보스 머리 위 HP 하트(pip: 남음 붉은색/소진 회색)와 피격 시 붉은 플래시(`_flashUntil`)를 그린다. 관통 피해는 `3 + floor(stage/20)`배로 완만히 상승.
