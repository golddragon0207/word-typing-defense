...
### 5. 📝 단어팩 & 라이브 제시어 길이 처리
* **기본 제시어 풀(`wordPacks.words`)**: 방송/게임/개발/음식·일상/밈/자연 등 6개 카테고리 72개 단어로 구성. **몬스터 제시어는 팩 구분 없이 전부 정확히 한글 6글자(최대 글자수)로 통일**해 밸런스를 균일화. 6글자 한글의 실측 평균 타수 ≈14.8타를 `SPAWN_CURVE.avgWordKeystrokes`=15와 짝맞춰 **스테이지1이 100타로 클리어되는 밸런스를 유지**한다(단어 길이를 바꾸면 이 상수도 함께 맞춰야 함). 별도 JSON/`fetch` 없이 배열 정적 관리. **한글 타자 중심 게임이라 영타 제시어는 두지 않는다.**
* 프리셋 팩 선택: `mixed`(기본 믹스), `memes`(방송 밈 24종), `hardcore`(억까 오타유발 10종), `spelling`(맞춤법 퀴즈 10종) + 모달 내 실시간 칩 미리보기(`renderWordPackPreview`). **몬스터 팩 제시어 6글자 통일**(memes도 6글자, 평균 ≈15타로 정합. hardcore 잰말놀이는 4~6자로 짧아도 같은 자모 반복이 많아 타수 ≈16타로 기본팩과 균일 — 입력 난이도만 매운맛). 단, `spelling`은 맞춤법 O/X 표기가 교육용 컨셉이라 글자수·특수문자 규칙에서 예외. (한글 중심 원칙에 따라 영타 팩은 제외.)
* 라이브 채팅 제시어: 시청자가 길게 치면 최대 글자수(**6자 고정**, `wordPacks.liveChatMaxLen`)로 **잘라서(truncate)** 사용. **띄어쓰기(공백)는 몇 칸인지 판정이 애매해 항상 전량 제거**하고, **한글(`가-힣`·자모)만 남기고 영문·숫자·이모티콘·특수문자를 전량 제거**(`liveChatStripSpecial=true` 고정) — 단어 중간 한/영 전환 억까를 막기 위함. (길이·정제 조정 UI는 §3대로 제거되어 값이 고정이다.)
...
* **보스 제시어 팩(`bossWords`, 8글자 통일 28종)**: 팩 난도 구분 없이 **모든 팩이 하나의 8글자 보스 세트를 공유**(몬스터 6글자보다 길게). `_pickBossWord(stage)`가 시스템 붕괴 테마 문구를 출제하며, 후반 스테이지일수록 획수 상위 문구로 좁힌다(`_bossPool`). 보스 차지 시간(`_bossChargeMs`)은 단어 타수에서 자동 역산.
