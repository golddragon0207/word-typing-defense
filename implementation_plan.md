...
* **홈 화면 가독성·세로 맞춤**: 홈 카드 폭은 `min(94%, 1000px)`, 높이는 상단바 아래 영역 전체인 `660px`로 두어 상단바 60px과 합쳐 720px을 모두 사용한다. 내부 콘텐츠는 카드 중앙에 정렬한다. 세로로 길게 쌓이던 설명서와 방송 연동 패널은 2열로 나란히 배치하고 각 본문을 최대 175px 내부 스크롤로 제한해, 기본 상태에서 시작 버튼과 728×90 광고까지 화면 안에 모두 표시한다. 게임 설명서는 첫 방문부터 내용을 볼 수 있도록 **기본 펼침** 상태다.
