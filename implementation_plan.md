...
* **홈 화면 가독성·세로 맞춤**: 홈 카드 폭은 `min(94%, 1000px)`로 두어 1280px 프레임의 약 78%를 차지하게 한다. 제목·부제·시작 버튼도 홈 화면에 한해 소폭 확대한다. 세로로 길게 쌓이던 설명서와 방송 연동 패널은 2열로 나란히 배치하고 각 본문을 최대 175px 내부 스크롤로 제한해, 기본 상태에서 시작 버튼과 728×90 광고까지 1280×720 화면 안에 모두 표시한다.
