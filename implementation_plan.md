# 🎮 word-typing-defense (스트리머 워드 디펜스) 구현 계획서 (v3.6)
...
> **v3.6 변경 요약**: **낙하·피버·보스 표시 감각 개선 묶음** — ① 제시어 스폰을 상단 HUD(스테이지창) '안'(`y:40`)에서 생성해 스테이지창 뒤에서 스르륵 내려오게 하고, 방어선을 소폭 하향(`groundY = height−160 → −130`, 548→578)해 낙하(반응) 구간을 넓힘(대포는 중앙 유지). ② **보스 스테이지에는 일반 몬스터(산성비) 스폰을 완전 차단**(`_spawnTick`에서 `_isBossStage`면 즉시 return) — 보스만 상대. ③ **피버 모드를 '화면 클리어 + 보너스'로 개편**: 6초 점수 2배를 폐지하고, 게이지 만땅 시 화면의 일반 몬스터를 한 번에 정리(`clearNonBoss`, 보스는 보존)하고 (정리분 점수 합 + 500) 보너스 + 기지 10% 회복(`triggerFeverBurst`) — '칠 게 없는 타이밍'에도 확실한 보상. ④ **스테이지업마다 5초 소환 딜레이**(`STAGE_UP_SPAWN_DELAY_MS`, `advanceStage`가 전달). ⑤ **제시어 난이도별 점수**: 일반 몬스터 점수를 `자모획수 × 6 × 스테이지`로 바꿔 어려운(획수 많은) 단어일수록 고득점(기본팩 평균 ≈16획이 스테이지1 ≈100점이 되도록 보정). ⑥ **보스 표시 또렷하게**: 남은 격파 수를 `HP ♥♥`(♥ 텍스트 글리프, 빨강=남음/회색=잃음)로 표시하고 차지 게이지 막대에 `CHARGE` 라벨을 붙여 각각 체력/차지임을 명확히(메커니즘은 v3.5 그대로).
>
> **v3.5 변경 요약**: **차지(기 모으기) 보스전으로 전면 개편** — 낙하 다중 HP 보스(v3.4)가 긴 보스 제시어(평균 43타)와 겹쳐 물리적으로 클리어 불가였던 문제(시뮬레이션 검증)를 해결. 보스는 낙하하지 않고 고정 위치에서 차지 게이지를 채우며, 게이지가 차기 전 정타로 격파하면 게이지 절반 밀어내기+새 제시어 교체, 필요 격파 수 달성 시 처치. 게이지 만땅 시 보스 공격(기지 정액 피해)+게이지 리셋(진행도 유지=항상 클리어 보장, 소프트락 없음). 스테이지 비례 스케일: 필요격파 `2+floor(st/20)`(≤5), 차지시간 `필요격파×6초`, 공격력 `10+floor(st/15)×3`, 후반 긴 제시어 우선. 렌더러에 차지 게이지 바+남은 격파 pip+공격 플래시. 보스 완전 처치 시 기지 체력 25% 회복(`healBase`). 시뮬레이션상 전 스테이지 클리어율 100%·보스 단독 사망 0.

> **v3.4 변경 요약(대체됨)**: 낙하형 다중 HP 보스 시도 — 긴 보스 제시어와 결합 시 난도가 과해 v3.5 차지 보스로 대체.
>
> **v3.3 변경 요약**: **게임 플레이 중 상단바 버튼 잠금** — 게임을 멈추지 않는 모달 버튼(📝 단어팩·🏆 명예의 전당·☕ 후원·💡 건의사항) 4개를 `PLAYING` 중 흐림 처리+클릭 차단(실수 클릭으로 게임 진행이 방치돼 지는 문제 방지). 라이브 채팅 모드·OBS·사운드는 방송 중 즉시 조정용이라 잠그지 않음. `StateManager.onStateChange` 콜백으로 상태 전환 시 `game.updateTopBarLock()` 토글. **보스 제시어 30종으로 확장**(기존 6 + 고난도 24, '시스템 붕괴' 테마, 길고 겹받침 많은 고난도 문구).
>
> **v3.2 변경 요약**: 상단바 브랜딩 정리 — 버전 배지 `SOLO STREAMER EDITION` 제거로 상단바가 **1024 폭에서 약 168px 여유**를 확보(브랜드 546→388px, 넘침 방지). 브랜드 이모지를 방어 컨셉에 맞춰 **🛡️로 통일**(상단바 로고 · 홈 히어로 타이틀 · 게임 무대 `🛡️ BASE DEFENSE LINE 🛡️` 방어선 라벨, 기존 ⚡ 대체).
>
...
*  **완만한 난이도 곡선(스테이지별 최소 클리어 타수 ≈ 50/60/70/80/90타…)**: 스테이지 간 난이도 점프를 최소화하도록 낙하·스폰·처치수를 함께 완만하게 상승시킨다(실제 단어팩 획수 분포 + 현실적 플레이 모델로 시뮬레이션 검증).
   *  낙하 속도: `speed = (0.30 + (stage-1)*0.04) * speedMult` (스테이지1 낙하 약 30초 — 낙하거리 538px÷18px/s, v3.6 방어선 하향으로 초반이 더 관대 → 초보도 여유 클리어, 이후 완만히 가속).
...
*  **포탑·방어선 하단 배치**: 하단 타자 입력 바(중앙 유지)가 대포를 가리지 않도록 포탑(`height−105`)과 방어선/몬스터 도달선(`groundY = height−130`, CanvasRenderer·MonsterManager 동일값 유지)을 배치. 방어선을 v3.6에서 소폭 하향(`−160 → −130`, 548→578)해 스폰 상향과 합쳐 낙하(반응) 구간을 넓혔다.
...
*  **🎯 제시어 난이도별 점수(자모 획수 비례)**: 일반 몬스터 점수를 `scoreValue = max(30, round(자모획수 × 6)) × 스테이지`로 산정(`MonsterManager.spawnMonster`). 어려운(길고 획수 많은) 단어일수록 높은 점수를 준다. 기본팩 평균(≈16획)이 스테이지1에서 ≈100점이 되도록 배수(6)를 보정해 기존 밸런스를 유지하면서 난이도 보상을 반영(예: 키보드 12획, 스트리머 16획, 간장공장공장장 32획 → 배수만큼 차등). 보스 점수는 별도(`500 × 스테이지`).
*  **콤보**: 명중 시 증가, 오타/피격 시 초기화. 최대 콤보 기록.
*  **🔥 피버 모드(화면 클리어 + 보너스)**: 콤보 누적으로 게이지가 차면 **피버 버스트** 발동 — 화면의 일반 몬스터를 한 번에 정리(`MonsterManager.clearNonBoss`, **보스는 보존**)하고 (정리분 점수 합 + 500) 보너스 점수(`addFeverBonus`) + 기지 체력 10% 회복(`healBase`)을 준다(`StateManager.triggerFeverBurst` → `game.js triggerFeverBurst`, 사운드/토스트/폭발 연출). 이후 게이지 리셋. (기존 '6초 점수 2배' 방식은 폐지 — 피버가 뜨는데 '칠 게 없는 타이밍'이면 보상이 낭비되던 문제를 해결.) 처치 수/스테이지 진행에는 반영하지 않음(점수·정리·회복만). **⚠️ 보스 스테이지(5의 배수)에서는 피버 게이지가 아예 차지 않는다**(`registerHit`이 `currentStage % 5 === 0`이면 누적 스킵) — 보스전엔 정리할 잡몹이 없어 버스트가 의미 없이 터지던 문제 방지(게이지는 그대로 다음 스테이지로 이월).
*  **⏸ 일시정지**(`GameEngine.togglePause`): 게임 진행 중 **ESC 키** 또는 **마우스(HUD 우측 `#btn-pause` / 오버레이 `#btn-pause-resume`)** 로 정지/재개(모달 열림 시 무시). 정지 중 몬스터 이동·스폰(`_spawnTick` 가드)·입력을 멈추고 오버레이(`#pause-overlay`) 표시. **정지 동안 흐른 시간을 `startTime`에 더해** 경과시간(=WPM 분모)에서 제외 → 화장실 등으로 잠깐 비워도 타수가 왜곡되지 않음. ⚠️ 스페이스바는 제시어(특히 라이브 채팅 문구)에 공백이 들어갈 수 있어 일시정지 키로 쓰지 않는다.

### 11. ♾️ 무한 Stage & 5 Stage 단위 보스전
*  처치목표(`killPerStageBase + floor((stage-1)*killPerStageStep)`) 달성 시 다음 스테이지로 진행.
*  **5 Stage마다 보스전**: WARNING 배너 → 보스 소환(확대·다중 피해). 보스 처치 시 즉시 다음 스테이지. **보스 스테이지에는 일반 몬스터(산성비)를 스폰하지 않는다** — `_spawnTick`이 `_isBossStage`면 즉시 return하여 보스 하나만 상대(보스 소환 후에도 일반 몬스터가 쏟아지던 버그 수정).
...
*  **⚡ 차지(기 모으기) 보스**(`spawnBoss`): 낙하하지 않고 고정 위치(`y:260`, `speed:0`)에서 차지 게이지를 채운다. 스테이지 스케일 — 필요격파 `requiredHits = 2 + floor(stage/20)`(≤5), 차지시간 `chargeTime = requiredHits × 6000ms`, 공격력 `attackDamage = 10 + floor(stage/15)×3`, 제시어는 `_pickBossWord(stage)`가 획수 티어에서 후반일수록 긴 문구 우선 출제.
    ...
    *  **렌더러(표시 또렷하게)**: 보스 머리 위에 ① **보스 HP** = `HP ♥♥` — 남은 격파 수를 ♥ 칸(U+2665 텍스트 글리프, 빨강=남음/회색=잃음)으로 그리고 `HP` 태그를 붙여 '체력'임을 명확히, ② **차지 게이지 바** = 막대 위에 `CHARGE` 라벨을 붙여 '공격 예열 게이지'임을 명확히(찰수록 노랑→빨강). 격파 시 분홍(`_flashUntil`)·공격 시 강한 빨강(`_attackFlashUntil`) 플래시. (v3.6에서 pip→HP♥·라벨 추가로 가독성 개선, 메커니즘은 동일)
...
*  **🌐 상위 %(글로벌 백분위)**: 게임오버 결과 화면 등급 뱃지 아래에 `상위 X%`를 표시(`GlobalLeaderboard.fetchPercentile`). 점수 기준이며, 이 compat SDK에 `count()` 집계가 없어 `where('score','>',0)`로 **0점 기록을 제외**하고 점수 내림차순으로 누적 기록을 `PERCENTILE_SCAN_CAP`(2000)까지 읽어 개수·순위를 직접 센다. **누적 기록 수가 `MIN_SAMPLE`(50) 미만이면 '집계 중' 표시**(표본 부족 시 무의미한 값 방지), 내 점수가 0이거나 Firebase 미설정/오프라인/오류면 뱃지 자동 숨김. 규칙 변경 불필요(읽기는 이미 공개). 대량 트래픽 단계에선 서버측 count/누적 카운터 문서로 교체 권장.
