...
### 5. 📝 단어팩 & 라이브 제시어 길이 처리
* **기본 제시어 풀(`wordPacks.words`)**: 방송/게임/개발/음식·일상/밈/자연/동물/장소 등 8개 카테고리 총 **126개** 6글자 단어로 구성. **몬스터 제시어는 전부 정확히 한글 6글자(최대 글자수)로 통일**해 밸런스를 균일화. 6글자 한글의 실측 평균 타수 ≈14.8타를 `SPAWN_CURVE.avgWordKeystrokes`=15와 짝맞춰 **스테이지1이 100타로 클리어되는 밸런스를 유지**한다(단어 길이를 바꾸면 이 상수도 함께 맞춰야 함). 별도 JSON/`fetch` 없이 배열 정적 관리. **한글 타자 중심 게임이라 영타 제시어는 두지 않는다.**
* **단어 모달팩 제거 및 제시어 다변화**: 선택형 프리셋 단어/닉네임 팩 모달을 삭제하고, 단일 6글자 제시어 풀(126종)로 통합하여 복잡한 설정 없이 게임 본연의 타자 재미와 균일한 난이도 밸런스를 전달.
