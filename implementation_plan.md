...
* **실제 연결 상태 표시**: URL 추가 직후에는 `연결 중`, 플랫폼 API 조회와 WebSocket/YouTube 폴링이 실제 성공하면 `연결됨`, 초기화 실패·소켓 오류는 `오류`, 연결 종료는 `끊김`으로 채널 칩과 전체 상태 배지에 표시한다. 연결 결과는 `SOOP URL 연동 실패`처럼 짧은 문구로 연동 목록 바로 아래에 항상 표시한다.
...
* **홈 화면 가독성·세로 맞춤**: 홈 배경은 상단바 아래 `660px` 전체를 사용하고, 폭 `min(94%, 1080px)`의 홈 카드는 상단바와 위아래 12px 간격을 둔다. 상단바에 이미 게임명이 있으므로 홈 카드 내부의 중복 대형 제목·부제와 채팅 탭 아래의 중복 플랫폼 배지를 제거한다. 설명서와 방송 연동 패널을 첫 행의 2열로 배치하고 문구·여백·입력 높이를 압축해, **1280×720에서도 두 패널 내부를 스크롤하지 않고 한 화면에서 모두 확인·설정**할 수 있다. 시작 버튼과 728×90 광고도 화면 안에 유지하며 게임 설명서는 **기본 펼침** 상태다.
...
* **720p 무스크롤 기본 화면**: 홈·게임·게임오버·후원·건의사항·TOP 5 모달은 위아래 이동 없이 한 화면에 표시한다. 모달 너비는 `min(96%, 800px)`, 높이는 최대 `640px`; 게임오버 통계는 가로 6칸으로 배치한다. 최대 200건의 전체 순위처럼 물리적으로 한 화면에 담을 수 없는 데이터 목록에만 목록 내부 스크롤을 허용한다. 모달과 토스트도 논리 프레임 내부에서 앱과 같은 비율로 스케일된다.
