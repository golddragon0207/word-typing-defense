...
| **방송 채팅 연동 방식** | **원클릭 방송 URL 파싱 연동**(방송 주소 입력 시 자동 ID 추출). SOOP/치지직 **동시 다중 연동**. SOOP·치지직 REST는 전용 무료 Cloudflare Worker 프록시(`CONFIG.SOOP_PROXY`/`CONFIG.CHZZK_PROXY`, 개발자 1회 배포) 경유 |
...
| **화면 & 모달** | 홈 카드 `width: min(94%, 1080px)`, 결과 카드 최대 960px·모달 카드 최대 800px / 모달 세로 `max-height: min(640px, calc(100% - 16px))`. 버튼 영역(`.modal-actions`)과 광고 영역(`.modal-footer`) 분리 |
...
### 1. 📡 원클릭 방송 URL 실시간 채팅 연동 (치지직/SOOP 다중)
...
* **다중 플랫폼 동시 연동**: `channels[]` 배열 구조로 SOOP + 치지직 동시 연결. 각 플랫폼 채팅은 플랫폼 접두사(🔵/🟢)와 함께 하나의 시청자 대기열로 병합. 연동 모달 기본 탭·시작화면 배지는 **SOOP 선두** 배치.
* **참여자 목록 표시**: `!참여`한 시청자를 홈 화면 채팅 연동 패널(`<details id="home-chat-panel">`)에 실시간 목록(총원 + 최근 참여자 칩)으로 표시.
* **실제 연결 상태 표시**: URL 추가 직후에는 `연결 중`, 플랫폼 API 조회와 WebSocket 연결이 실제 성공하면 `연결됨`, 초기화 실패·소켓 오류는 `오류`, 연결 종료는 `끊김`으로 채널 칩과 전체 상태 배지에 표시한다. 연결 결과는 `SOOP URL 연동 실패`처럼 짧은 문구로 연동 목록 바로 아래에 항상 표시한다.
...
* **CORS 프록시 / 웹소켓**: SOOP·치지직 REST API는 **하나의 무료 Cloudflare Worker 프록시**([`proxy/soop-cors-proxy.worker.js`](proxy/soop-cors-proxy.worker.js), SOOP/아프리카 + `api.chzzk.naver.com`·`comm-api.game.naver.com` 도메인 허용) 경유; 채팅 웹소켓은 양쪽 다 브라우저에서 직접 연결.
...
* ⚠️ SOOP·치지직은 `CONFIG.SOOP_PROXY`/`CONFIG.CHZZK_PROXY`(같은 Worker 주소) 필요(미설정 시 BOT 폴백).
...
* **프레임 독립 게임 루프**: `requestAnimationFrame`의 실제 타임스탬프로 `deltaTime`을 계산해 30·60·120Hz에서도 동일한 속도를 유지한다. 탭 복귀 시 한 프레임 순간이동을 막기 위해 최대 50ms로 제한하고, 플레이 중에만 게임 캔버스를 업데이트·렌더링한다. 스타필드는 30FPS로 제한하며 `visibilitychange`로 숨겨진 탭 또는 OBS 투명 모드에서는 중단한다.
...
* **OBS 크로마키 가시성**: 텍스트 두꺼운 아웃라인(Stroke) + Drop Shadow. OBS 버튼이 `body.obs-overlay` 클래스를 토글해 배경을 투명화하며, 보이지 않는 스타필드 애니메이션도 동시에 정지·재개한다.
...
3. **`js/config.js`**
   * `CONFIG.SOOP_PROXY`/`CONFIG.CHZZK_PROXY`/`CONFIG.SOOP_DEBUG`, `CONFIG.FIREBASE`, `CONFIG.KAKAO_ADFIT`(5개), `CONFIG.DIFFICULTY`(밸런스 테이블) + `getDifficultyConfig()`, `CONFIG.MAX_MONSTER_CAP`(15), `CONFIG.START_SPAWN_DELAY_MS`(5000)/`STAGE_UP_SPAWN_DELAY_MS`(3000)/`RESUME_GRACE_MS`(5000), `CONFIG.QUEUE` 튜닝값, 광고 리프레시 로직.
