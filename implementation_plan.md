...
| **명예의 전당** | **최고 도달 스테이지 기준 단일 TOP 5**(동점 시 점수순). 로컬(`localStorage`) 기본 + Firebase Firestore **글로벌 리더보드**(설정 시 자동 활성, 미설정 시 로컬 폴백) + 전체 순위 보기 토글(최대 200위). **본인 기록 행 시안 하이라이트(`나` 태그) + 전체보기 시 내 등수로 자동 스크롤** |
...
* **두 모드 모두 상단 뱃지는 `!참여` 시청자 닉네임**을 그대로 쓰고, **차이는 하단 제시어를 어디서 뽑느냐**뿐이다(`getNextMonsterData`의 `word: chatWord || randomWord` / `isLiveChat: !!chatWord`).
  ...
  | :--- | :--- | :--- | :--- |
  | **라이브 OFF(기본)** | `!참여` 시청자 닉네임 | 단어팩 랜덤 단어(`getActiveWords`) | 일반(팩) 색 (`rgb(168, 0, 52)`) |
...
* **명예의 전당**: 최고 도달 스테이지 내림차순(동점 시 점수순). 로컬 `localStorage`(`wtd_leaderboard_top5`) 기본 + Firebase Firestore(`leaderboard`). 전체 순위 보기 버튼(`#btn-leaderboard-all`)으로 TOP 5 ↔ 전체(최대 200위) 토글.
  * **🙋 본인 기록 강조**: 현재 스트리머 닉네임(`getMyNickname` — 홈 입력값 우선, 없으면 `config.playerNames[0]`, 기본값 `'스트리머'`는 미설정 처리)과 일치하는 행에 `.is-me` 시안 하이라이트 + `나` 태그를 붙이고, 전체보기에서는 내 기록을 리스트 내부에서 화면 중앙으로 자동 스크롤(모달 밖으로 넘치지 않음).
