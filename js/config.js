...
const CONFIG = {
  ...
  //      요구 타자속도 ≈ (60000 ÷ 스폰주기ms) × 단어당타수(~15, 한글 6글자). → 목표 타자속도에서 스폰주기를 역산한다.
  ...
  //    - avgWordKeystrokes 15 : 단어팩 평균 타수. **몬스터 제시어를 한글 6글자로 통일**했고 그 실측 평균(≈14.8타)에
  //         맞춘 값. ⚠️ 이 값은 임의 목표치가 아니라 "실제 단어 평균 타수"와 반드시 일치해야 요구 타수(스테이지1=100타)가
  //         정확해진다. 단어 길이를 바꾸면(예: 4글자로) 이 상수도 그 평균으로 다시 맞춰야 한다.
  //    ※ 하한 400ms는 극후반 안전장치일 뿐(6글자 기준 요구타수 2325타 이상에서야 도달 — 사실상 미발동).
  //    ※ 튜닝: 초반 관대/빡빡=kpmStart, 상승 기울기=kpmStep, 속도 목표 천장=kpmMax, 후반 조임세기=kpmStepAfterMax.
  SPAWN_CURVE: { kpmStart: 100, kpmStep: 10.5, kpmMax: 800, kpmStepAfterMax: 3, avgWordKeystrokes: 15 },
