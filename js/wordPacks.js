...
const wordPacks = {
  ...
  //    ⚠️ 밸런스: 몬스터 제시어는 **전부 정확히 한글 6글자로 통일**(최대 글자수 기준).
  //       6글자 한글의 실측 평균 타수 ≈ 14.8타라, config의 SPAWN_CURVE.avgWordKeystrokes=15와 짝이 맞아
  //       스테이지1이 100타로 클리어되는 밸런스가 유지된다(단어 길이가 바뀌면 이 상수도 함께 맞춰야 함).
  //    🎲 제시어는 매번 이 한 풀에서 랜덤으로 뽑힌다(선택형 팩 없음). 다양성이 곧 재미이므로
  //       카테고리를 넓게 섞고 배열만 늘리면 된다 — 정적 사이트라 fetch 없이 즉시 반영된다.
  //       ✅ 추가 규칙: (1) 반드시 정확히 한글 6글자, (2) 완성형(가-힣)만, (3) 비속어/영문/숫자 금지.
  words: [
    // 📺 방송/스트리밍
    "실시간생방송", "게임스트리머", "인기스트리머", "방송사고발생", "실시간채팅창", "구독자이벤트",
    "후원해주세요", "오늘도방송중", "다시보기영상", "시청자와소통", "방송종료인사", "별풍선감사해",
    "생방송시작함", "도네이션감사", "채팅창불났네", "오늘도풀방송", "댓글읽어주기", "합방켜주세요",

    // 🎮 게임/타자
    "크리티컬발동", "일격필살기술", "보스몬스터전", "스피드클리어", "게임오버직전", "하드코어모드",
    "노데스클리어", "최종보스등장", "리스폰대기중", "레벨업축하해", "만렙달성완료", "콤보이어가기",
    "리겜한판더요", "장비강화실패", "파밍노가다중", "랭크게임승급", "히든보스발견", "아이템줍줍중",

    // 💻 개발/기술
    "자바스크립트", "데이터베이스", "네트워크오류", "알고리즘문제", "브라우저게임", "프로그래머님",
    "메모리초과됨", "서버다운발생", "코드리뷰완료", "버그수정완료", "배포준비완료", "캐시삭제하기",
    "깃허브푸시함", "리팩토링지옥", "런타임에러남", "테스트통과함", "무한루프주의", "깜빡이는커서",

    // 🍜 음식/일상
    "김치찌개백반", "삼겹살구이집", "떡볶이오뎅탕", "마라탕세트요", "치킨한마리요", "붕어빵한봉지",
    "야식으로치킨", "배달음식주문", "곱창전골한상", "라면끓여먹기", "카페라떼한잔", "아이스아메리",
    "야식은역시빵", "국밥한그릇뚝", "달달한디저트", "냉면곱빼기요", "탕수육부먹파", "김밥천국단골",

    // 😆 밈/유행어(가벼운 것)
    "완전레전드각", "어쩔티비저쩔", "폼미쳤다진짜", "럭키비키잔셀", "갓생살기시작", "나만아니면돼",
    "완전킹받네요", "이게바로국룰", "실화냐진짜로", "무야호외치기", "개추놀리기각", "완전찐친인정",
    "구독과좋아요", "오타내지마라", "개같이부활각", "멘탈바사삭각", "능지폭발직전", "내로남불진짜",
    "참교육당했다", "알빠임아니오", "실수연발주의", "뇌절그만하기", "좋댓구독필수", "웃음벨눌렸다",

    // 🌤️ 자연/생활 명사
    "무지개뜬하늘", "첫눈오는날에", "자전거하이킹", "바닷가산책로", "단풍구경가자", "벚꽃놀이가기",
    "별똥별떨어져", "소나기내린날", "고양이산책중", "강아지랑산책", "여행가방싸기", "손난로따뜻해",
    "노을진저녁길", "밤하늘별구경", "산들바람분다", "새벽공기상쾌", "빗소리들으며", "함박눈내린다",

    // 🐾 동물/캐릭터
    "고양이가르릉", "강아지꼬리춤", "팬더는대나무", "수달손잡고자", "토끼귀쫑긋쫑", "다람쥐도토리",

    // 🏙️ 여행/장소
    "제주도한바퀴", "부산가는기차", "한강에서치맥", "벚꽃엔딩거리", "야경명소투어", "온천여행가자"
  ],
