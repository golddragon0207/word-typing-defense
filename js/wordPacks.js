...
const wordPacks = {
  ...
  presetPacks: {
    ...
    // 🟢 일반 티어(몬스터 4글자 이하 한글): 기본 words와 동일한 밸런스로 뽑힌다.
    memes: [
    ...
    // 🔴 어려움 티어(잰말놀이 컨셉 유지 + 8글자 상한): 보스도 긴 버전(bossWordsHard) 사용.
    hardcore: [
      "간장공장공장장", "경찰청철창살", "저기저뜀틀은",
      "내가그린기린그림", "서울특별시민",
      "앞집팥죽뒷집콩죽", "신라면과진라면", "칠월칠일",
      "홍삼홍삼황홍삼", "육개장과닭개장"
    ],
    // 🟡 특수 티어(맞춤법 O/X 컨셉): 글자수·X/O 표기는 교육용이라 규칙에서 예외로 둔다.
    spelling: [
      "어의없다X어이없다O", "몇일X며칠O", "왠지O웬지X",
      "설레임X설렘O", "돼다X되다O", "웬만하면", "역할O역활X",
      "안돼요O안되요X", "찌개O찌게X", "금세O금새X"
    ],
    // 🔴 어려움 티어(영타 컨셉 + 8글자 상한): 보스도 긴 버전(bossWordsHard) 사용.
    english: [
      "compiler", "canvas", "keyboard", "terminal", "debugger",
      "streamer", "defense", "critical", "combo", "victory"
    ]
  },

  // 1-2. 팩 난도 티어 — 보스 단어 세트/밸런스 분기에 사용.
  //      'hard'로 지정된 팩은 보스도 긴 버전(bossWordsHard)을 쓴다. 미지정 팩은 'normal'.
  packDifficulty: {
    mixed: 'normal',
    memes: 'normal',
    spelling: 'normal',
  ...
  //    ⚠️ 밸런스: 전부 정확히 6글자 한글로 통일해 보스전 난도(차지 게이지)를 균일하게 맞춘다.
  //    '시스템 붕괴' 테마를 유지하되 소재를 다양하게 섞어 반복 체감을 줄인다.
  bossWords: [
    // 🖥️ 서버/인프라 붕괴
    "시스템대붕괴", "네트워크마비", "서버전면다운", "커널패닉발생",
    "트래픽대폭발", "최종방어붕괴", "데이터전손실", "메모리누수됨",
    // 🔒 보안/침투
    "백신무력화됨", "루트권한탈취", "암호화키유출", "백도어침투됨",
    "권한상승탐지", "랜섬웨어감염", "침입탐지실패", "보안체계뚫림",
    // ⚙️ 시스템 과부하/오작동
    "캐시붕괴발생", "과부하로폭주", "코어온도폭발", "백업서버다운",
    "동기화충돌됨", "무한루프폭주", "방열판정지됨", "제어권상실됨",
    // 🚨 긴급/최종
    "긴급복구실패", "통신교란확산", "최종병기가동", "전원공급차단"
  ],

  // 2-1. 어려움 티어(hardcore·english) 전용 보스 팩 — 전부 9글자로 통일해 일반(6글자)보다 길다.
  //      획수/타수가 자동으로 커져 보스 차지 시간(_bossChargeMs)도 함께 늘어난다.
  bossWordsHard: [
    // 🖥️ 서버/인프라 붕괴
    "네트워크접속초과됨", "커널패닉연쇄폭발됨", "백업서버전면다운됨",
    "데이터베이스전손실", "무한루프과부하폭주", "동기화충돌연쇄발생",
    // 🔒 보안/침투
    "루트권한전량탈취됨", "암호화키전량유출됨", "권한상승공격탐지됨",
    "랜섬웨어전면감염됨", "백신엔진강제무력화", "분산서비스거부공격",
    // ⚙️ 시스템 과부하/오작동
    "방어체계전면붕괴됨", "코어온도임계돌파됨", "버퍼오버플로우발생",
    "방화벽전면동시붕괴",
    // 🚨 긴급/최종
    "긴급복구시퀀스실패", "통신프로토콜교란됨", "최종병기강제가동됨",
    "자폭시퀀스강제개시"
  ],
  ...
  applyPresetPack(packKey) {
    ...
    // 현재 팩 키를 기억해 보스 단어 세트(getActiveBossWords)가 난도 티어를 참조할 수 있게 한다.
    this._activePackKey = packKey;
  },

  /**
   * 🐲 현재 팩 난도에 맞는 보스 단어 배열 반환
   *    - 'hard' 팩(hardcore·english): 9글자 긴 보스 세트(bossWordsHard)
   *    - 그 외: 6글자 일반 보스 세트(bossWords)
   *    (MonsterManager가 보스 스폰·차지 계산 시 이 메서드를 참조한다)
   * @returns {string[]}
   */
  getActiveBossWords() {
    const tier = this.packDifficulty[this._activePackKey] || 'normal';
    if (tier === 'hard' && Array.isArray(this.bossWordsHard) && this.bossWordsHard.length > 0) {
      return this.bossWordsHard;
    }
    return this.bossWords;
  },
