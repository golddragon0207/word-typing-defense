...
const wordPacks = {
  ...
  //    (별도 JSON 없이 여기 배열만 늘리면 정적 사이트에서 fetch 없이 즉시 반영된다. 추가 단어도 6글자로.)
  words: [
    // 📺 방송/스트리밍
  ...
  presetPacks: {
    ...
    // 🟢 몬스터 팩은 전부 6글자로 통일(밸런스 균일화). 기본 words와 동일 규칙(평균 ≈15타).
    memes: [
      "구독과좋아요", "오타내지마라", "스트리머능지", "개같이부활각", "나만아니면돼", "멘탈바사삭각",
      "무야호외치기", "갓생살기시작", "완전킹받네요", "이게바로국룰", "능지폭발직전", "내로남불진짜",
      "참교육당했다", "알빠임아니오", "방송종료각각", "실수연발주의", "치트키금지령", "멘붕오기직전",
      "개꿀잼인정합", "뇌절그만하기", "억까자제부탁", "좋댓구독필수", "완전레전드각", "폼미쳤다진짜"
    ],
    // 잰말놀이(오타 유발) 컨셉. 글자수는 4~6자로 짧아도 같은 자모 반복이 많아 타수는 ≈16타로
    // 기본팩(≈15타)과 비슷해 밸런스가 균일하다(입력 난이도만 '매운맛').
    hardcore: [
      "간장공장장", "경찰청철창살", "저기저뜀틀은",
      "내가그린기린", "서울특별시민",
      "앞집콩죽팥죽", "신라면진라면", "칠월칠일",
    ...
    // 🇰🇷 영타(english) 팩은 제거 — 한글 타자 중심 게임이라 한/영 전환을 강요하는 영문 제시어는 제외한다.
  },
  ...
  /**
   * 프리셋 팩 적용 (모달 > 단어팩 선택)
   * @param {string} packKey - 'mixed' | 'memes' | 'hardcore' | 'spelling'
   */
