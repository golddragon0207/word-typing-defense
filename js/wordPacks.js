...
const wordPacks = {
  ...
  //    ⚠️ 밸런스: 몬스터 제시어는 **3글자(words3) / 4글자(words4) 두 풀로 분리**한다.
  //       스테이지가 오를수록 4글자 비중이 선형으로 커진다(초반=3글자 위주 → 후반=4글자 위주).
  //       · 4글자 확률 p4 = clamp((stage-4)/11, 0, 1)  → s1~4=순수 3글자, s15+=순수 4글자, 그 사이 선형 혼합.
  //       · getNextMonsterData(_, _, stage)가 이 확률로 풀을 골라 단어를 뽑는다.
  //    🎯 실측 평균 타수: 3글자 ≈ 7.66타, 4글자 ≈ 10.14타. config의 SPAWN_CURVE.avgWordKeystrokes=8.9는
  //       이 둘의 중간값이다. **단일 평균 고정** 방식이라, 요구 타수 곡선(문서화된 KPM)은 그대로 두고
  //       초반 3글자(평균보다 짧음)는 실제로 더 쉽고 / 후반 4글자(평균보다 김)는 실제로 더 빡세게 만든다
  //       (= "초반 쉽고 후반 빡센" 곡선을 글자수로 얹는다). 풀 구성을 바꾸면 이 8.9(중간값)도 다시 맞춰야 함.
  //    🎲 제시어는 매번 해당 스테이지 풀에서 랜덤으로 뽑힌다(선택형 팩 없음). 다양성이 곧 재미이므로
  //       카테고리를 넓게 섞고 배열만 늘리면 된다 — 정적 사이트라 fetch 없이 즉시 반영된다.
  //       ✅ 추가 규칙: (1) words3는 정확히 3글자·words4는 정확히 4글자, (2) 완성형(가-힣)만, (3) 비속어/영문/숫자 금지.
  words3: [
    // 📺 방송/스트리밍
    "시청자", "구독자", "별풍선", "채팅창", "생방송", "방송국", "유튜브",
    // 🎮 게임/타자
    "아이템", "궁극기", "필살기", "방어력", "공격력", "회복약", "레벨업", "리스폰", "무적기", "보스방",
    // 💻 개발/기술
    "데이터", "깃허브", "오탈자", "코딩각", "버그각", "배포각",
    // 🍜 음식/일상
    "떡볶이", "삼겹살", "마라탕", "붕어빵", "탕수육", "비빔밥", "순대국", "육개장", "갈비탕", "감자탕",
    "청국장", "부침개", "계란찜", "어묵탕", "만두국",
    // 😆 밈/유행어
    "레전드", "무야호", "국룰각", "존버각", "갓생각", "찐친각", "개꿀각", "실화냐", "참교육", "알빠노", "뇌절각", "개추각",
    // 🌤️ 자연/생활
    "무지개", "소나기", "함박눈", "별똥별", "잠자리", "반딧불", "무궁화", "도토리", "민들레", "진달래", "개나리", "봉숭아",
    // 🐾 동물
    "고양이", "강아지", "다람쥐", "햄스터", "앵무새", "코끼리", "원숭이", "호랑이", "너구리", "병아리", "두꺼비", "개구리", "올챙이",
    // 🏙️ 여행/장소
    "제주도", "지하철", "자전거", "킥보드", "놀이터", "목욕탕", "찜질방", "도서관", "백화점", "편의점", "영화관", "노래방"
  ],
  words4: [
    // 📺 방송/스트리밍
    "스트리머", "도네이션", "다시보기", "방송사고", "인기방송", "채팅도배", "방송종료", "알림설정", "후원감사",
    // 🎮 게임/타자
    "보스전투", "크리티컬", "스피드런", "하드코어", "최종보스", "만렙달성", "랭크게임", "히든보스", "장비강화", "게임오버", "부활대기", "궁극기술",
    // 💻 개발/기술
    "알고리즘", "네트워크", "브라우저", "버그수정", "코드리뷰", "서버다운", "무한루프", "리팩토링", "캐시삭제", "빌드실패",
    // 🍜 음식/일상
    "김치찌개", "삼겹살집", "떡볶이집", "배달음식", "곱창전골", "카페라떼", "냉면정식", "김밥천국", "순대국밥", "부대찌개", "닭갈비집", "제육덮밥", "돈까스집",
    // 😆 밈/유행어
    "갓생시작", "럭키비키", "어쩔티비", "좋댓구독", "완전인정", "찐친인증", "능지폭발", "국룰인정",
    // 🌤️ 자연/생활
    "벚꽃엔딩", "단풍구경", "산들바람", "새벽공기", "노을풍경", "반딧불이", "눈꽃송이",
    // 🐾 동물
    "고슴도치", "카멜레온",
    // 🏙️ 여행/장소
    "제주여행", "부산여행", "온천여행", "야경투어", "캠핑여행", "기차여행", "벚꽃놀이", "단풍여행", "바다구경", "시장구경"
  ],

  // 2. 보스전 전용 단어 팩 (Stage 5, 10, 15...)
  //    ⚠️ 밸런스: 5~6글자 한글로 통일(몬스터 3~4글자보다 길게)해 보스전을 스파이크로 만든다.
  //    '시스템 붕괴' 테마를 유지하되 소재를 다양하게 섞어 반복 체감을 줄인다.
  //    (팩 난도 구분 없이 모든 팩이 이 한 세트를 공유 — 차지 시간은 _bossChargeMs가 타수로 자동 역산.
  //     _bossPool이 획수 오름차순 정렬 후 후반 스테이지일수록 긴(6글자) 문구로 좁힌다.)
  bossWords: [
    // 🖥️ 서버/인프라 붕괴 (5글자)
    "서버다운됨", "시스템붕괴", "메모리누수", "방화벽붕괴", "권한탈취됨", "트래픽폭발", "캐시붕괴됨", "코어과열됨", "전원차단됨",
    // 🖥️ 서버/인프라 붕괴 (6글자)
    "네트워크마비", "서버전면다운", "커널패닉발생", "시스템붕괴됨", "트래픽대폭발", "데이터손실됨",
    // 🔒 보안/침투 (6글자)
    "백신무력화됨", "루트권한탈취", "암호화키유출", "백도어침투됨", "랜섬웨어감염", "침입탐지정지", "보안체계돌파",
    // ⚙️ 시스템 과부하/오작동 (6글자)
    "코어온도돌파", "백업서버다운", "방화벽붕괴됨", "제어권한상실",
    // 🚨 긴급/최종 (6글자)
    "긴급복구실패", "최종병기가동", "전원공급차단"
  ],
  ...
  liveChatMode: false,
  liveChatMaxLen: 4,         // 라이브 채팅 문구 최대 글자수 (일반 몬스터 최대치인 4글자에 맞춰 상한 — 5자↑ 아웃라이어 차단, 조정 UI 없음)
  ...
  /**
   * 🎯 해당 스테이지의 4글자 제시어 출제 확률.
   *    p4 = clamp((stage-4)/11, 0, 1)  → s1~4=0(순수 3글자), s15+=1(순수 4글자), 그 사이 선형 상승.
   *    (초반 3글자 온보딩 → s15에서 4글자 위주로 완성. 스케줄을 바꾸려면 이 한 줄만 조정.)
   * @param {number} stage
   * @returns {number} 0~1 사이 4글자 확률
   */
  getWord4Probability(stage) {
    const s = Number(stage) || 1;
    return Math.max(0, Math.min(1, (s - 4) / 11));
  },
   ...
   * - 단어팩 단어는 스테이지에 따라 3글자(words3)/4글자(words4) 풀에서 확률적으로 선택
   * @param {string|null} customNickname - 강제 지정 닉네임 (선택)
   * @param {Set<string>|null} excludeWords - 이미 화면에 떠 있는 제시어 집합(랜덤 단어 중복 회피용, 선택)
   * @param {number} stage - 현재 스테이지 (3/4글자 풀 선택 확률에 사용, 기본 1)
   * @returns {Object} { nickname, isBot, word, isLiveChat }
   */
  getNextMonsterData(customNickname = null, excludeWords = null, stage = 1) {
    ...
    // 🎯 스테이지 확률로 3글자/4글자 풀 선택 (초반=3글자 위주 → 후반=4글자 위주).
    //    한쪽 풀이 비어 있으면 다른 풀로 안전 폴백.
    const useWord4 = Math.random() < this.getWord4Probability(stage);
    let activeWords = (useWord4 ? this.words4 : this.words3);
    if (!Array.isArray(activeWords) || activeWords.length === 0) {
      activeWords = (this.words4 && this.words4.length) ? this.words4 : this.words3;
    }
